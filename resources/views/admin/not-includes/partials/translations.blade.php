            ...
            @foreach(\App\Locale::all() as $locale)
                <div class="tab-pane {{$locale->locale == 'en' ? 'active' : ''}}" id="tab_{{$locale->locale}}">
                    ...
                    <div class="form-group  {{ $errors->has($locale->locale.'.title') ? 'has-error' : '' }}">
                        ...
                        <input class="form-control" name="{{$locale->locale}}[title]" value="{{ @$notInclude ? $notInclude->translate($locale->locale)->title : old($locale->locale.'.title' ,optional($locale->locale)->{'title:'.$locale->locale}) }}" placeholder="@lang('general.title')">
                    </div>
                    <div class="form-group  {{ $errors->has($locale->locale.'.description') ? 'has-error' : '' }}">
                        <label>{{ __('general.description') }} *</label>
                        <input class="form-control" name="{{$locale->locale}}[description]" value="{{ @$notInclude ? $notInclude->translate($locale->locale)->description : old($locale->locale.'.description' ,optional($locale->locale)->{'description:'.$locale->locale}) }}" placeholder="@lang('general.description')">
                    </div>
